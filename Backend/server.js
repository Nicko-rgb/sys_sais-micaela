  ...
  try {
    ...
    if (searchTerm) {
      ...
      searchTerms.forEach((term) => {
        conditions.push(
          `(LOWER(dni) LIKE ? OR LOWER(hist_clinico) LIKE ? OR LOWER(CONCAT(ape_paterno, ' ', ape_materno, ', ', nombres)) LIKE ?)`
        );
...
app.post("/api/registrar/pacientes", async (req, res) => {
  ...
  try {
    ...
    res
      .status(500)
      .json({ message: "Error al guardar el paciente", error: error.message });
  } finally {
...
app.post("/api/register/personal-salud", async (req, res) => {
  ...
  try {
    ...
    const [results] = await connection.query(checkEmailQuery, [
      dataPersonal.correo,
    ]);
    ...
    const insertQuery =
      "INSERT INTO personal_salud (dni, paterno, materno, nombres, tipo_user, profesion, servicio, especial_cita, condicion, celular, correo, usuario, contrasena) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)";
    ...
    res
      ...
      .json({
        ...
        id_personal: insertResult.insertId,
      });
  } catch (error) {
    ...
    return res
      .status(500)
      .json({ message: "Error al registrar el personal. Intenta nuevamente." });
  } finally {
...
app.post("/api/sais/login", async (req, res) => {
  try {
    ...
    const [rows] = await pool.execute(
      "SELECT * FROM personal_salud WHERE usuario = ?",
      [username]
    );
    ...
    if (rows.length === 0) {
      return res
        .status(401)
        .json({ message: "Usuario o contraseña incorrectos (usuario)" });
    }
    ...
    if (contrasena !== personal.contrasena) {
      return res
        .status(401)
        .json({ message: "Usuario o contraseña incorrectos (contraseña)" });
    }
...
app.post("/api/recover-password", async (req, res) => {
  ...
  try {
    ...
    const [user] = await connection.execute(
      "SELECT * FROM personal_salud WHERE correo = ?",
      [email]
    );
    ...
    await connection.execute(
      "UPDATE personal_salud SET reset_token = ? WHERE correo = ?",
      [token, email]
    );
    ...
    res
      .status(500)
      .json({ message: "Error al enviar el correo de recuperación." }); // Enviar error
  } finally {
    connection.release(); // Liberar la conexión
  }
});
...
app.post("/api/reset-password/:token", async (req, res) => {
  ...
  try {
    ...
    const [user] = await connection.execute(
      "SELECT * FROM personal_salud WHERE reset_token = ?",
      [token]
    );
    ...
    await connection.execute(
      "UPDATE personal_salud SET contrasena = ?, reset_token = NULL WHERE reset_token = ?",
...
app.post("/api/registrar/cita-nino", (req, res) => {
  ...
  pool.query(sql, values, (error, results) => {
    ...
    res
      .status(201)
      .json({ message: "Cita registrada exitosamente", id: results.insertId });
  });
});
// Fin de la ruta para registrar las citas para el niño

//ENDPONTS PARA EDITAR  LOS DATOS DEL PACIENTE
